...
这一集里面，我们主要带大家走入 Linux 命令行的精彩世界。首先带大家选择一款合适的 Linux 操作系统，演示一下如何配置命令行终端软件，然后宏观介绍一下应该如何学习各种各样命令。学 Linux 应该说主要就是学命令行，强大的命令行界面是比用鼠标来回比划比划要强一百万倍的利剑，是开发者必备的利剑。相比之下 Windows 下的 dos 批处理就太简陋了，开发中根本没法用。
